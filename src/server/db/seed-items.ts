/**
 * Authored catalogue of item bases + loot materials — the seed data for the `items` table. This is
 * the "world-DB content" for items (the MaNGOS / TrinityCore `item_template` equivalent): edit/add
 * rows here (or directly in the DB) to change the gear and materials. content.ts loads the rows at
 * runtime and the simulation reads only from there; nothing imports this array during the game.
 *
 * Weapons give power; armor gives HP; jewelry leans on affixes. Each entry maps to one `items` row.
 * The slot TYPES and UI labels stay in src/shared/equipment.ts (shared contract); only the DATA
...
export const MATERIALS: Record<string, { name: string; color: string }> = {
