...
/** Every EQUIPMENT base whose slot matches `slot` (the pool a gamble pull draws from). */
function basesForSlot(slot: string): readonly BaseItem[] {
...
/** True if `slot` is an ItemSlot that has at least one base item to gamble for. */
export function isGambleSlot(slot: string): boolean {
  return basesForSlot(slot).length > 0;
}

/**
 * Roll a random gambled item for the given equip slot: pick a random EQUIPMENT base whose `slot`
 * matches, then roll a normal instance (random rarity via {@link rollItemInstance} — gambling can
 * hit rare/epic just like a drop). Returns null if `slot` has no base item. `uid` is the unique id
 * to assign; `rng` is injected for determinism.
 */
