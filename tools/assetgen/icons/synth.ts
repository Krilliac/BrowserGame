...
/** Draw an item icon. `tint` overrides the rarity-derived accent (gem family colors, coin gold, …). */
function drawIcon(
  r: Raster,
  cell: number,
  kind: IconKind,
  ...
  const gem: RGBA = tint ?? [rim[0], rim[1], rim[2], 255];
  switch (kind) {
    case 'sword':
    ...
    case 'amulet':
      ...
      break;
    case 'mace':
      r.line(c, cell * 0.85, c, cell * 0.34, cell * 0.045, WOOD);
      r.disc(c, cell * 0.28, cell * 0.13, STEEL);
      for (let i = 0; i < 6; i++) {
        const a = (i / 6) * Math.PI * 2;
        r.disc(
          c + Math.cos(a) * cell * 0.13,
          cell * 0.28 + Math.sin(a) * cell * 0.13,
          1.6,
          shade(STEEL, -0.1),
        );
      }
      break;
    case 'dagger':
      r.line(c, cell * 0.78, c, cell * 0.3, cell * 0.04, STEEL);
      r.polygon(
        [
          [c - 1.5, cell * 0.3],
          [c + 1.5, cell * 0.3],
      ...
      r.fillRect(c - cell * 0.1, cell * 0.72, cell * 0.2, cell * 0.04, GOLD);
      break;
    case 'spear':
      r.line(c, cell * 0.9, c, cell * 0.18, cell * 0.04, WOOD);
      r.polygon(
        [
          [c - 2.5, cell * 0.3],
          [c + 2.5, cell * 0.3],
      ...
      break;
    case 'gloves':
      r.fillRect(cell * 0.34, cell * 0.42, cell * 0.32, cell * 0.28, shade(STEEL, -0.05));
      r.fillRect(cell * 0.38, cell * 0.3, cell * 0.08, cell * 0.16, shade(STEEL, -0.05)); // a finger
      r.fillRect(cell * 0.54, cell * 0.3, cell * 0.08, cell * 0.16, shade(STEEL, -0.05));
      break;
    case 'boots':
      r.fillRect(cell * 0.38, cell * 0.3, cell * 0.16, cell * 0.34, shade(WOOD, 0.1)); // shaft
      r.fillRect(cell * 0.38, cell * 0.6, cell * 0.34, cell * 0.12, shade(WOOD, -0.05)); // sole/toe
      break;
    case 'legs':
      r.fillRect(cell * 0.34, cell * 0.28, cell * 0.14, cell * 0.44, shade(STEEL, -0.08));
      r.fillRect(cell * 0.52, cell * 0.28, cell * 0.14, cell * 0.44, shade(STEEL, -0.08));
      break;
    case 'tome':
      r.fillRect(cell * 0.28, cell * 0.26, cell * 0.44, cell * 0.48, shade(WOOD, -0.1));
      r.fillRect(cell * 0.3, cell * 0.28, cell * 0.4, cell * 0.44, [225, 210, 170, 255]);
      r.disc(c, c, cell * 0.08, gem); // a clasp gem
      break;
    case 'coin':
      r.disc(c, cell * 0.56, cell * 0.2, GOLD);
      r.disc(c - 3, cell * 0.5, cell * 0.18, shade(GOLD, 0.1));
      r.disc(c - 6, cell * 0.44, cell * 0.16, shade(GOLD, 0.18));
      break;
    case 'rune':
      r.polygon(
        [
      ...
      r.line(c, cell * 0.36, c, cell * 0.64, 1.2, shade(gem, 0.4)); // a glyph stroke
      r.line(c - 4, cell * 0.5, c + 4, cell * 0.44, 1.2, shade(gem, 0.4));
      break;
    case 'material':
      r.disc(c, cell * 0.56, cell * 0.18, shade(gem, -0.1));
      r.disc(c + 5, cell * 0.5, cell * 0.1, shade(gem, 0.1));
      r.disc(c - 5, cell * 0.6, cell * 0.08, shade(gem, 0.05));
      break;
  }
}
...
/** Render a packed icon sheet for (id → kind, rarity, optional tint) entries. The cell map is keyed
 *  by `id` — pass kind names as ids for a kind-keyed sheet the engine resolves item ids onto. */
export function synthIcons(src: string, cell: number, entries: IconEntry[]): IconSheet {
  const perRow = 8;
  ...
  entries.forEach((e, i) => {
    ...
    drawIcon(tile, cell, e.kind, e.rarity, e.tint);
